#!/usr/bin/env python3
"""Apply a release: bump the [package] version in Cargo.toml + the matching
entry in Cargo.lock, and prepend the new section to CHANGELOG.md.
...
Usage:
    apply-release.py <version> <section_file> <cargo_toml> <cargo_lock> <changelog>
"""
...
def main(argv: list[str]) -> int:
    if len(argv) != 6:
        sys.exit(f"usage: {argv[0]} <version> <section_file> <cargo_toml> <cargo_lock> <changelog>")
    _, version, section_path, cargo_path, lock_path, changelog_path = argv
